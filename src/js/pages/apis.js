...
import environments from '../data/mock-environments.json';
      ...
      <div class="col-12 col-md-6 col-xl-4">
        <button type="button" class="card card--interactive api-card text-body w-100 h-100 text-start" data-api="${api.id}">
          <span class="api-card__head">
            ...
            <span class="badge badge-neutral ms-auto">v${escapeHtml(api.version)}</span>
          </span>
          <span class="api-card__desc">${escapeHtml(api.description)}</span>
          <span class="api-card__meta">
            <span class="badge badge-status badge-status--${STATUS_BADGE[api.status]}"><span class="dot"></span>${escapeHtml(api.status)}</span>
            <span class="text-tertiary caption">${api.endpointsCount} endpoints</span>
          </span>
        </button>
      </div>`
    )
  ...
  return list
    .map(
      ...
      <tr class="is-clickable" tabindex="0" data-endpoint="${ep.id}">
        ...
        <td class="text-secondary">${escapeHtml(API_NAMES[ep.apiId] || ep.group)}</td>
      </tr>`
    )
    .join('');
}
...
function renderEndpointList(filter = '') {
  ...
  tbody.innerHTML = list.length
    ? endpointRows(list)
    : `<tr><td colspan="4"><div class="empty-state"><span class="empty-icon"><i data-lucide="search"></i></span><h4 class="empty-title">No endpoints match</h4><p class="empty-desc mb-0">Try a different path or keyword.</p></div></td></tr>`;
  createIcons({ icons });
}
  ...
  return `
    <div class="code-block code-block--flush" data-code-block>
      <div class="code-block__header">
        ...
        <div class="code-block__actions"><button type="button" class="btn btn-icon btn-icon--sm" data-code-copy aria-label="Copy ${label}"><i data-lucide="copy"></i></button></div>
      </div>
  ...
  return `
    <div class="code-block" data-code-block>
      <div class="code-block__header">
        ...
        <div class="code-block__actions"><button type="button" class="btn btn-icon" data-code-copy aria-label="Copy code"><i data-lucide="copy"></i></button></div>
      </div>
...
function paramRows(ep) {
  if (!ep.params.length) return `<p class="text-secondary">This endpoint takes no parameters.</p>`;
  return `
          ...
          <tbody>${ep.params
            .map(
              (p) => `<tr>
                ...
                <td>${p.required ? '<span class="param-required">required</span>' : '<span class="text-tertiary">optional</span>'}</td>
...
function renderDoc(ep) {
  ...
  doc.innerHTML = `
    ...
    <h4 class="h4 mb-2">Parameters</h4>
    <div class="mb-4">${paramRows(ep)}</div>
    <h4 class="h4 mb-2">Code examples</h4>
    <div class="mb-4">${sdkBlock(ep)}</div>
    <h4 class="h4 mb-2">Response</h4>
    <div>${jsonBlock(ep.responseExample, '200 response')}</div>`;
  ...
  return `
    <div class="tester__param">
      <label class="tester__param-label" for="param-${escapeHtml(p.name)}">
        ...
        <span class="text-tertiary caption">${escapeHtml(p.type)}${p.required ? ' · required' : ''}</span>
      </label>
...
function renderTester(ep) {
  ...
  tester.innerHTML = `
    <div class="d-flex align-items-center justify-content-between">
      <h4 class="mb-0">Request builder</h4>
      <span class="badge badge-neutral">${escapeHtml(API_NAMES[ep.apiId] || ep.group)}</span>
    </div>
    <div class="tester__url">
      <span class="tester__method">${ep.method}</span>
      <input class="form-control form-control--mono ltr-isolate" dir="ltr" value="${escapeHtml(baseUrlFor(currentEnv()) + ep.path)}" readonly aria-label="Request URL" />
    </div>
    ${bodyParams.length ? `<div class="d-flex flex-column gap-3">${bodyParams.map(inputFor).join('')}</div>` : `<p class="text-tertiary caption mb-0">This endpoint takes no parameters.</p>`}
    <button type="button" class="btn btn-primary w-100" id="tester-send"><i data-lucide="play"></i> Send request</button>
    <div class="response-viewer" id="tester-response" aria-live="polite">
      <div class="response-viewer__status text-tertiary caption">Send a request to see the response.</div>
    </div>`;
  ...
  send.addEventListener('click', () => {
    ...
    setTimeout(() => {
      send.disabled = false;
      send.innerHTML = '<i data-lucide="play"></i> Send request';
      ...
      if (missing.length) {
        responseEl.innerHTML = `
          <div class="response-viewer__status">
            ...
            <span class="response-viewer__latency">${latency}ms</span>
          </div>
          ${jsonBlock({ error: { code: 'invalid_request', message: `Missing required parameter${missing.length > 1 ? 's' : ''}: ${missing.map((m) => m.name).join(', ')}.` } }, '400 response')}`;
      } else {
        responseEl.innerHTML = `
          <div class="response-viewer__status">
          ...
          ${jsonBlock(ep.responseExample, '200 response')}`;
      }
