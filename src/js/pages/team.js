...
import invitationsData from '../data/mock-invitations.json';
...
function renderSummary() {
  const currentPlan = plans.find((p) => p.name === 'Scale') || plans[plans.length - 1];
  const used = activeMembers().length;
  document.getElementById('team-plan').textContent = currentPlan.name;
  document.getElementById('team-seats').textContent = `${used} of ${currentPlan.members} seats used`;
    ...
    <tr data-id="${m.id}">
      <td>
        <div class="member">
          ...
          <span class="member__meta">
            <span class="member__name">${escapeHtml(m.name)}${isOwner ? ' <span class="text-tertiary caption">(you)</span>' : ''}</span>
      ...
      <td class="text-end">
        <div class="row-actions dropdown" data-dropdown>
          <button type="button" class="btn btn-icon btn-icon--sm" data-bs-toggle="dropdown" aria-label="Actions for ${escapeHtml(m.name)}" aria-expanded="false">
            <i data-lucide="more-horizontal"></i>
          </button>
          <div class="dropdown-menu dropdown-menu-end">
            ${!isOwner ? roleOptions : ''}
            ${!isOwner && m.status === 'active' ? `<button type="button" class="dropdown-item" data-suspend><i data-lucide="ban"></i> Suspend</button>` : ''}
            ${m.status === 'suspended' ? `<button type="button" class="dropdown-item" data-activate><i data-lucide="circle-check"></i> Re-activate</button>` : ''}
            ${!isOwner ? `<hr class="dropdown-divider" /><button type="button" class="dropdown-item is-danger" data-remove><i data-lucide="trash-2"></i> Remove</button>` : ''}
          </div>
        </div>
      </td>
    </tr>`;
      ...
      <td>
        <div class="member">
          ...
          <span class="member__meta">
            ...
            <span class="member__email">Invited by ${escapeHtml(inv.invitedBy)}</span>
          </span>
        </div>
      </td>
      <td>${roleBadge(inv.role)}</td>
      <td>${statusBadge('pending')}</td>
      <td class="text-secondary">Sent ${relativeTime(inv.sentAt)}</td>
      <td class="text-secondary">Expires in ${escapeHtml(inv.expiresIn)}</td>
      <td class="text-end">
        <button type="button" class="btn btn-sm btn-ghost" data-resend><i data-lucide="send"></i> Resend</button>
        <button type="button" class="btn btn-icon btn-icon--sm" data-revoke aria-label="Revoke invitation"><i data-lucide="x"></i></button>
      </td>
    </tr>`;
}
...
function render() {
  document.getElementById('team-list').innerHTML = members.length
    ...
    : `<tr><td colspan="6"><div class="empty-state"><span class="empty-icon"><i data-lucide="users"></i></span><h4 class="empty-title">No members</h4><p class="empty-desc mb-0">Invite your first teammate to start collaborating.</p></div></td></tr>`;
  ...
  invWrap.innerHTML = inv.length
    ...
    : `<tr><td colspan="6"><div class="empty-state"><span class="empty-icon"><i data-lucide="mail"></i></span><h4 class="empty-title">No pending invitations</h4><p class="empty-desc mb-0">New invitations will appear here until they are accepted or expire.</p></div></td></tr>`;

  document.getElementById('team-count').textContent = `${members.length} member${members.length === 1 ? '' : 's'}`;
  document.getElementById('team-seats').textContent = `${activeMembers().length} of ${(plans.find((p) => p.name === 'Scale') || {}).members} seats used`;
  createIcons({ icons });
}
...
function bindActions() {
  document.getElementById('team-list').addEventListener('click', async (e) => {
    ...
    if (actionBtn.dataset.role) {
      ...
      if (ok) {
        ...
        render();
        afxToast({ message: `${member.name} removed.`, type: 'info' });
      }
    }
  });
  ...
  document.getElementById('team-invitations').addEventListener('click', (e) => {
    ...
    if (e.target.closest('[data-resend]')) {
      ...
      render();
      afxToast({ message: `Invitation re-sent to ${inv.email}.`, type: 'success' });
    } else if (e.target.closest('[data-revoke]')) {
      inv.revoked = true;
      render();
      afxToast({ message: `Invitation to ${inv.email} revoked.`, type: 'info' });
    }
  });
}
