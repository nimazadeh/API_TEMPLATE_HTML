...
const STATUS = {
  delivered: { key: 'webhooks.statusDelivered', cls: 'badge-status--success' },
  failed: { key: 'webhooks.statusFailed', cls: 'badge-status--error' },
  retrying: { key: 'webhooks.statusRetrying', cls: 'badge-status--warning' },
  pending: { key: 'webhooks.statusPending', cls: 'badge-status--info' },
};

const TIMELINE_STATES = {
  created: { key: 'webhooks.timelineCreated', icon: 'clock', cls: 'is-neutral' },
  sent: { key: 'webhooks.timelineSent', icon: 'arrow-up-right', cls: 'is-info' },
  delivered: { key: 'webhooks.timelineDelivered', icon: 'circle-check', cls: 'is-success' },
  failed: { key: 'webhooks.timelineFailed', icon: 'alert-circle', cls: 'is-error' },
  retrying: { key: 'webhooks.timelineRetrying', icon: 'rotate-ccw', cls: 'is-pending' },
};
    ...
    <div class="code-block code-block--flush">
      <div class="code-block__header">
        ...
        <div class="code-block__actions">
          <button type="button" class="btn btn-icon btn-icon--sm" data-copy data-copy-target="#${codeId}" aria-label="${t('aria.copyX', { name: escapeHtml(title) })}"><i data-lucide="copy"></i></button>
        </div>
      </div>
...
function timelineItem(entry) {
  ...
  const metaBits = [];
  metaBits.push(clockTime(entry.at));
  ...
  return `
    <li class="timeline__item">
      ...
      <div class="timeline__content">
        <div class="timeline__title">${t(state.key)}</div>
  ...
  body.innerHTML = `
    <div class="inspector-head">
      <div class="d-flex align-items-center gap-2 flex-wrap">
        ...
        <span class="badge badge-status ${status.cls}"><span class="dot"></span>${t(status.key)}</span>
      ...
      <div class="d-flex align-items-center gap-2 mt-3">
        ...
        <button type="button" class="btn btn-sm btn-secondary" data-delivery-action="replay"><i data-lucide="play"></i> ${t('webhooks.replayEvent')}</button>
        <button type="button" class="btn btn-sm btn-ghost" data-delivery-action="copy-payload"><i data-lucide="copy"></i> ${t('webhooks.copyPayload')}</button>
      </div>
    </div>

    <section class="inspector-section">
      <h4 class="inspector-label">${t('webhooks.deliveryTimeline')}</h4>
      ${delivery.timeline && delivery.timeline.length
        ? `<ul class="timeline">${delivery.timeline.map(timelineItem).join('')}</ul>`
        : `<div class="text-secondary caption">${t('webhooks.queued')}</div>`}
    </section>

    <section class="inspector-section">
      <h4 class="inspector-label">${t('webhooks.payloadInspector')}</h4>
      <ul class="nav nav-pills mb-3" role="tablist" aria-label="${t('webhooks.payloadInspector')}">
        <li class="nav-item" role="presentation">
          <button class="nav-link active" id="wd-tab-payload" data-bs-toggle="tab" data-bs-target="#wd-pane-payload" type="button" role="tab" aria-controls="wd-pane-payload" aria-selected="true"><i data-lucide="braces"></i> ${t('webhooks.tabPayload')}</button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="wd-tab-headers" data-bs-toggle="tab" data-bs-target="#wd-pane-headers" type="button" role="tab" aria-controls="wd-pane-headers" aria-selected="false"><i data-lucide="file-text"></i> ${t('inspector.headers')}</button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="wd-tab-response" data-bs-toggle="tab" data-bs-target="#wd-pane-response" type="button" role="tab" aria-controls="wd-pane-response" aria-selected="false"><i data-lucide="terminal"></i> ${t('inspector.response')}</button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="wd-tab-signature" data-bs-toggle="tab" data-bs-target="#wd-pane-signature" type="button" role="tab" aria-controls="wd-pane-signature" aria-selected="false"><i data-lucide="shield-check"></i> ${t('webhooks.tabSignature')}</button>
        </li>
      </ul>
      <div class="tab-content">
        <div class="tab-pane fade show active" id="wd-pane-payload" role="tabpanel" aria-labelledby="wd-tab-payload">
          ${jsonWell(t('webhooks.eventPayload'), 'file-json', 'wd-payload-code', payloadText)}
        </div>
          ...
          ${delivery.response
            ? `
              <div class="d-flex align-items-center gap-2 mb-2">
                ...
                <span class="text-secondary caption">${t('webhooks.httpResponseBody')}</span>
              </div>
              ${jsonWell(t('inspector.responseBody'), 'terminal', 'wd-response-code', responseText)}`
            : `<div class="empty-state py-4">
                <span class="empty-icon"><i data-lucide="clock"></i></span>
                <h4 class="empty-title">${t('webhooks.awaitingResponse')}</h4>
                <p class="empty-desc mb-0">${t('webhooks.awaitingResponseDesc')}</p>
              </div>`}
        </div>
        <div class="tab-pane fade" id="wd-pane-signature" role="tabpanel" aria-labelledby="wd-tab-signature">
          <div class="code-block code-block--flush mb-2">
            <div class="code-block__header">
              <span class="code-block__lang"><i data-lucide="shield-check"></i> ${t('webhooks.tabSignature')}</span>
              <div class="code-block__actions">
                <button type="button" class="btn btn-icon btn-icon--sm" data-copy data-copy-target="#wd-signature-code" aria-label="${t('aria.copySignature')}"><i data-lucide="copy"></i></button>
              </div>
            </div>
